                   ...
                   double Ac[11], double x[12])
{
  ...
  dv2[6] = 1.0;
  for (i2 = 0; i2 < 7; i2++) {
    dv3[i2] = dv1[i2] - dv2[i2];
  }

  for (k = 0; k < 7; k++) {
    Kp[k] = 0.0;
    for (i2 = 0; i2 < 7; i2++) {
      Kp[k] += a[k + 7 * i2] * dv3[i2];
    }
  ...
  while (err_N_n > 0.001) {
    ...
    err_sq_p_o2 = 1.0;
    dsq_p_o2 = 0.0;
    while (err_sq_p_o2 > 0.001) {
      sq_p_o2 += dsq_p_o2;
      Ksq4 = Kp[3] * sq_p_o2;
      Ksq5 = Kp[4] * sq_p_o2;
      Ksq6 = Kp[5] * sq_p_o2;
      Ksq7 = Kp[6] * sq_p_o2;
      sq_p_h2_0 = 8.0 * N_h_o * N_o_sol;
      y = Ksq4 + Kp[0];
      sq_p_h2_1 = sqrt(sq_p_h2_0 * (1.0 + Ksq6) + y * y);
      sq_p_h2_2 = 4.0 * (Ksq6 + 1.0);
      sq_p_h2 = -((Kp[0] + Ksq4) - sq_p_h2_1) / sq_p_h2_2;
      dsq_p_h2 = Kp[5] * -sq_p_h2 / (sq_p_h2_2 / 4.0) - (Kp[3] - 0.5 * (2.0 *
        Kp[3] * (Kp[0] + Ksq4) + Kp[5] * sq_p_h2_0) / sq_p_h2_1) / sq_p_h2_2;
      y = Ksq5 + Kp[2];
      sq_p_n2_1 = sqrt(y * y + 8.0 / N_o_n * N_o_sol);
      sq_p_n2 = ((sq_p_n2_1 - Ksq5) - Kp[2]) / 4.0;
      N_o_sol2_0 = N_c_o * N_o_sol;
      y = -((((((N_o_sol2_0 * (2.0 - 1.0 / (Ksq7 + 1.0)) + 2.0 * (sq_p_o2 *
                  sq_p_o2)) + Ksq6 * (sq_p_h2 * sq_p_h2)) + Ksq4 * sq_p_h2) +
              Kp[1] * sq_p_o2) + Ksq5 * sq_p_n2) - N_o_sol) / ((((((((N_o_sol2_0
        * Kp[6] / ((Ksq7 + 1.0) * (Ksq7 + 1.0)) + 4.0 * sq_p_o2) + Kp[5] *
        (sq_p_h2 * sq_p_h2)) + Ksq6 * 2.0 * sq_p_h2 * dsq_p_h2) + Kp[3] *
        sq_p_h2) + Ksq4 * dsq_p_h2) + Kp[1]) + Kp[4] * sq_p_n2) + Ksq5 * (((Ksq5
        + Kp[2]) * Kp[4] / sq_p_n2_1 - Kp[4]) / 4.0));
      varargin_2 = -0.9 * sq_p_o2;
      if ((y >= varargin_2) || rtIsNaN(varargin_2)) {
        dsq_p_o2 = y;
      } else {
        dsq_p_o2 = varargin_2;
      }

      err_sq_p_o2 = fabs(dsq_p_o2 / sq_p_o2);
    }
    ...
    Ksq6 = Kp[5] * sq_p_o2;
    y = Ksq4 + Kp[0];
    sq_p_h2 = -((Kp[0] + Ksq4) - sqrt(8.0 * N_h_o * N_o_sol * (1.0 + Ksq6) + y *
      y)) / (4.0 * (Ksq6 + 1.0));
    y = Ksq5 + Kp[2];
    sq_p_n2 = ((sqrt(y * y + 8.0 / N_o_n * N_o_sol) - Ksq5) - Kp[2]) / 4.0;
    p_co2 = N_c_o * N_o_sol / (1.0 / (Kp[6] * sq_p_o2) + 1.0);
    p_h2 = sq_p_h2 * sq_p_h2;
    dN_n_temp = (((((((((((P - p_co2) - p_co2 / (Kp[6] * sq_p_o2)) - sq_p_o2 *
                         sq_p_o2) - Ksq6 * p_h2) - p_h2) - Ksq4 * sq_p_h2) - Kp
                     [0] * sq_p_h2) - Kp[1] * sq_p_o2) - sq_p_n2 * sq_p_n2) -
                  Kp[2] * sq_p_n2) - Ksq5 * sq_p_n2) - N_ar_o * N_o_sol;
    err_N_n = fabs(dN_n_temp / N_n_temp);
  }
  ...
  Ksq6 = Kp[5] * sq_p_o2;
  y = Ksq4 + Kp[0];
  sq_p_h2 = -((Kp[0] + Ksq4) - sqrt(8.0 * N_h_o * N_o_sol * (1.0 + Ksq6) + y * y))
    / (4.0 * (Ksq6 + 1.0));
  y = Ksq5 + Kp[2];
  sq_p_n2 = ((sqrt(y * y + 8.0 / N_o_n * N_o_sol) - Ksq5) - Kp[2]) / 4.0;
  p_co2 = N_c_o * N_o_sol / (1.0 / (Kp[6] * sq_p_o2) + 1.0);
...
/* End of code generation (GetEquilGrill.cpp) */
