  ...
  double W, double rho_v_data[], int rho_v_size[1])
{
  ...
  while (V_R_error > 0.001) {
    ...
    c41 = 0.042724 / TT_data[3] / VV[1];
    y = 0.060167 / VV[1];
    c43 = exp(-0.060167 / VV[1]);
    varargin_1 = 0.1 * V_R;
    y = V_R - (p_R / T_R * V_R - ((((1.0 + B_data_idx_0 / V_R) + C_data_idx_0 /
      VV[1]) + D / VV[4]) + c41 * (0.65392 + y) * c43)) / (p_R / T_R -
      (((((-B_data_idx_0 / VV[1] - 2.0 * C_data_idx_0 / VV[2]) - 5.0 * D / VV[5])
         + -0.085448 / TT_data[3] / VV[2] * (0.65392 + y) * c43) + c41 *
        (-0.120334 / VV[2]) * c43) + c41 * (0.65392 + y) * (0.120334 / VV[2] *
      c43)));
  ...
  while (V_R_error > 0.001) {
    ...
    c41 = 0.041577 / TT_data[3] / VV[1];
    y = 0.03754 / VV[1];
    c43 = exp(-0.03754 / VV[1]);
    varargin_1 = 0.1 * V_R;
    y = V_R - (p_R / T_R * V_R - ((((1.0 + B_h_data_idx_0 / V_R) +
      C_h_data_idx_0 / VV[1]) + D_h / VV[4]) + c41 * (1.226 + y) * c43)) / (p_R /
      T_R - (((((-B_h_data_idx_0 / VV[1] - 2.0 * C_h_data_idx_0 / VV[2]) - 5.0 *
                (4.8736E-5 + z) / VV[5]) + -0.083154 / TT_data[3] / VV[2] *
               (1.226 + y) * c43) + c41 * (-0.07508 / VV[2]) * c43) + c41 *
             (1.226 + y) * (0.07508 / VV[2] * c43)));
    if ((varargin_1 >= y) || rtIsNaN(y)) {
      V_R_temp = varargin_1;
    } else {
      V_R_temp = y;
  ...
  rho_v_data[0] = 1.0 / ((z0 + W / 0.3978 * (p_R * V_R / T_R - z0)) * 8314.0 * T
    / p) * MW;
}

/* End of code generation (GetDensityHCVapor.cpp) */
