 ...
 * KpNASA7_upper.cpp
 *
 * Code generation for function 'KpNASA7_upper'
 *
 */
...
void KpNASA7_upper(double T, double Kp_NASA7[7])
{
  ...
  dv5[6] = 1.0;
  for (i6 = 0; i6 < 7; i6++) {
    dv6[i6] = dv4[i6] - dv5[i6];
  }

  for (k = 0; k < 7; k++) {
    Kp_NASA7[k] = 0.0;
    for (i6 = 0; i6 < 7; i6++) {
      Kp_NASA7[k] += a[k + 7 * i6] * dv6[i6];
    }

    Kp_NASA7[k] = exp(Kp_NASA7[k]);
  }
}

/* End of code generation (KpNASA7_upper.cpp) */
