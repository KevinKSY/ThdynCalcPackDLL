  ...
  real_T *Cv, real_T *K)
{
  ...
  FC6F = FC4F * VF * FC6 / FC5;
  FC6T = -0.001 * FC6 * ((0.008868 - c_y) + 1.0) / TT;
  FC7 = PP / TT;
  FC7P = 1.01972E-5 / TT;
  FC7T = -FC7 / TT * 0.001;
  z = (0.008868 - c_y) / FC5;
  ...
  /* FC7T.*(FC2.*FC6.*(FC8 + 1) - FC3)./TT + FC7.*(FC2.*FC6T.*(FC8 + 1) + FC2.*FC6.*FC8T)./TT - ZZT1./TT*1e-3 */
  /*  ZZP1	= (FC3 - FC2.*FC6)./TT; */
  ...
  /*  FC31 = -1.0698./((1 + 0.0698*F).*(1 + 0.0698*F)); */
  *R = EP4 * FC4F;
  *h = EP4 * T * EP3;
  *s = EP4 * FC4;
  *u = *h - *R * T;

  /*  RF1 = FC11.*(ZZV.*FC31-ZZ.*FC21); */
  *RF = EP2 * FC4F + EP4 * (FC7 * ((FC3F - FC2F * FC6) - FC2 * FC6F));

  /*  RP1 = FC11.*ZZP1.*FC0; */
  *RP = EP4 * (FC7P * (FC3 - FC2 * FC6));

  /*  RT1 = FC11.*ZZT1*1.0e-3; */
  *RT = EP4 * (-FC7 * ((FC3 - FC2 * FC6) * 0.001 / TT + FC2 * FC6T));

  /*  hT1 = FC11.*(HH + TT.*HHT1); */
  /*  uF1 = FC11.*T.*((HHV-ZZV).*FC31-(HH-ZZ).*FC21); */
  *uF = (EP2 * T * EP3 + EP4 * T * ((-0.005026 + TT * (-0.383504 + TT *
            (0.185214 + TT * (-0.0694862 + TT * (0.0164041 + TT * (-0.00204537 +
    TT * 0.00010161)))))) * VF - FC7 * (((FC2F * FC6 * ((1.0 + z) + 1.0) +
             (0.000277105 - y) * FC6F * ((1.0 + z) + 1.0)) + (0.000277105 - y) *
            FC6 * (FC8F + 1.0)) - FC3F) / TT * TT)) - *RF * T;

  /*  uP1 = FC11.*T.*(HHP1-ZZP).*FC0; */
  *uP = EP4 * T * (-ZZT1 * TT / PP * 1.01972E-5) - *RP * T;

  /*  uT1 = FC11.*(HH + TT.*HHT1) - FC11.*ZZ - FC11.*ZZT1; */
